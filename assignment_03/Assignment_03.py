...
class Product:
    ...
    def __init__(self, name, price, category):
        if not name: raise ValueError("Product name cannot be empty.")
        if price < 0: raise ValueError("Product price cannot be negative.")
        if not category: raise ValueError("Product category cannot be empty.")
        self.name = name
        self.price = price
...
class OrderItem:
    """A line item that has a Product and a quantity."""
    def __init__(self, product, quantity):
        if not isinstance(product, Product): raise TypeError("product must be a Product.")
...
class Silver(Customer):
    @property
    def tier_name(self): return "silver"
    @property
    def points_multiplier(self): return 2
    def discount_rate(self, subtotal):
...
class Gold(Customer):
    @property
    def tier_name(self): return "gold"
    @property
    def points_multiplier(self): return 3
    def discount_rate(self, subtotal):
...
class Platinum(Customer):
    @property
    def tier_name(self): return "platinum"
    @property
    def points_multiplier(self): return 5
    def discount_rate(self, subtotal):
...
class Order:
    ...
    def __init__(self, customer_name, customer, items):
        if not customer_name: raise ValueError("Customer name cannot be empty.")
        if not isinstance(customer, Customer): raise TypeError("customer must be a Customer.")
        if not items: raise ValueError("An order must contain at least one item.")
        if not all(isinstance(item, OrderItem) for item in items):
            raise TypeError("items must contain only OrderItem objects.")
...
def _customer_from_tier(tier):
    tier_classes = {"none": Customer, "silver": Silver, "gold": Gold, "platinum": Platinum}
    try:
        return tier_classes[tier]()
    except KeyError:
        raise ValueError("Unknown customer tier: " + str(tier))
...
def _build_orders():
    ...
    for name, tier, raw_items in ORDERS:
        items = [OrderItem(products[product_index], quantity) for product_index, quantity in raw_items]
        orders.append(Order(name, _customer_from_tier(tier), items))
    return orders
